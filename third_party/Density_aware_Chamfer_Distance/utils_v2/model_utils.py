...
def calc_scd(x, gt, alpha=1000, n_lambda=1, return_raw=False, non_reg=False):
    x = x.float()
    gt = gt.float()
    ...
    weight1 = (torch.log(weight1) + 1 + 1e-6) ** (-1) * frac_21
    # loss1 = (1 - exp_dist1 * weight1).mean(dim=1)
    # loss1 = (dist1 * weight1).mean(dim=1)
    loss1 = (dist1 * weight1).sum(dim=1)
    ...
    weight2 = (torch.log(weight2) + 1 + 1e-6) ** (-1) * frac_12
    # loss2 = (1 - exp_dist2 * weight2).mean(dim=1)
    # loss2 = (dist2 * weight2).mean(dim=1)
    loss2 = (dist2 * weight2).sum(dim=1)
    ...
    res = [loss, cd_p, cd_t]
    if return_raw:
        res.extend([f1, dist1, dist2, idx1, idx2])
...
def calc_dcd(x, gt, alpha=1000, n_lambda=1, return_raw=False, non_reg=False):
    ...
    # loss1 = (1 - exp_dist1 * weight1).mean(dim=1)
    loss1 = (1 - exp_dist1 * weight1).sum(dim=1)
    ...
    # loss2 = (1 - exp_dist2 * weight2).mean(dim=1)
    loss2 = (1 - exp_dist2 * weight2).sum(dim=1)
