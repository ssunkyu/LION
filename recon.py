...
from utils.demo_utils import (
    ...
    get_loss_function,
    load_data, # We are redefining this function below
    load_data_random, # We are redefining this function below
)
...
def run(cfg: dict, model_cfg: dict, args: argparse.Namespace):
    ...
    # 4. Load data using the new DataLoader-based method
    x0 = load_data_random(cfg, device)
    if x0 is None:
        logging.error("Failed to load data. Aborting.")
        return

    # 5. Run DPS sampling
    # y_measurement = noiser.forward(y_measurement)
    logging.info("Running DPS sampling...")
    
    # This seems to be a reconstruction test, not DPS sampling.
    # To run actual sampling, you would call something like:
    # output = lion.sample(y_measurement, clip_feat, ...)
    output = lion.vae.recont(x0)

    # 6. Save results
    logging.info("Saving results...")
    pts = output['final_pred']
    
    save_point_cloud(x0, os.path.join(output_dir, "01_gt"))
    save_point_cloud(pts.detach(), os.path.join(output_dir, "03_recon"))
...
def main():
    ...
    parser.add_argument('--config', type=str, default='./config/ip_cfg.yml', help="Path to the base config file.")
