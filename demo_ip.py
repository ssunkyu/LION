...
def run(cfg: dict, model_cfg: dict, args: argparse.Namespace):
    ...
    exp_name = f"scale_{cfg['hyperparameters']['guidance_scale']}_start_t_{cfg['hyperparameters']['guidance_start_t']}_sched_{cfg['hyperparameters']['guidance_scheduler']}_loss_{loss_names}"
    ...
    clip_feat = prepare_text_conditioning(cfg, model_cfg, device)

    # 4. Load data
    x0 = load_data(cfg, device)
    if x0 is None:
    ...
    output = lion.dps_sample(
        ...
        clip_feat=clip_feat,
        debug=cfg['hyperparameters']['debug_mode']
    )
